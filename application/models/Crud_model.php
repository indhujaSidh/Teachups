...
class Crud_model extends CI_Model
{
    ...
    public function update_frontend_settings()
    {
        ...
        $data['value'] = $this->input->post('add_comment');
        $this->db->where('key', 'add_comment');
        $this->db->update('frontend_settings', $data);

        $data['value'] = $this->input->post('fetch_comment');
        $this->db->where('key', 'cookie_policy');
        $this->db->update('fetch_comment', $data);
        ...
        $data['value'] = $this->input->post('forum');
        $this->db->where('key', 'forum');
        $this->db->update('frontend_settings', $data);

        $data['value'] = $this->input->post('contactus');
        $this->db->where('key', 'contactus');
        $this->db->update('frontend_settings', $data);

        $data['value'] = $this->input->post('kidslearningzone');
        $this->db->where('key', 'kidslearningzone');
        $this->db->update('frontend_settings', $data);

        $data['value'] = $this->input->post('kidsvideo');
        $this->db->where('key', 'kidsvideo');
        $this->db->update('frontend_settings', $data);
        
        $data['value'] = $this->input->post('alphabeticallearning');
        $this->db->where('key', 'alphabeticallearningt');
        $this->db->update('frontend_settings', $data);

        $data['value'] = $this->input->post('quiz');
        $this->db->where('key', 'quiz');
        $this->db->update('frontend_settings', $data);

        $data['value'] = $this->input->post('mathsquiz1');
        $this->db->where('key', 'mathsquiz1');
        $this->db->update('frontend_settings', $data);

        $data['value'] = $this->input->post('kidspuzzle');
        $this->db->where('key', 'kidspuzzle');
        $this->db->update('frontend_settings', $data);

        $data['value'] = $this->input->post('science_forum');
        $this->db->where('key', 'science_forum');
        $this->db->update('frontend_settings', $data);
    ...
    public function update_forum_banner()
    {
        if (isset($_FILES['forum_image']) && $_FILES['forum_image']['name'] != "") {
            unlink('uploads/system/' . get_frontend_settings('forum_image'));
            $data['value'] = md5(rand(1000, 100000)) . '.jpg';
            $this->db->where('key', 'forum_image');
            $this->db->update('frontend_settings', $data);
            move_uploaded_file($_FILES['forum_image']['tmp_name'], 'uploads/system/' . $data['value']);
        }
    }
